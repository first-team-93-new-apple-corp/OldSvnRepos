 ...
 *
 */
public class RightLeftBehind extends CommandGroup
{
	
	public RightLeftBehind()
	{
		addSequential(new SleepCommand(SmartDashboard.getNumber("TimeBeforeAuto", 0)));
		addSequential(new TilterDown());
		addSequential(new ScalerMove());
		addSequential(new DriveEncoderTicks(
				Drive.LENGTHTOSWITCHINFEET / 2 + Drive.LENGTHTOSWITCHINFEET * Drive.TICKSPERFOOT));
		addSequential(new TurnDeg(-90, 3));
		addSequential(new DriveEncoderTicks(Drive.WIDTHOFSWITCHINFEET * Drive.TICKSPERFOOT));
		addSequential(new TurnDeg(90, 3));
