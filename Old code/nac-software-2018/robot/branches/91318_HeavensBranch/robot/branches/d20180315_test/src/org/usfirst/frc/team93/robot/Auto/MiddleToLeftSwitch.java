...
public class MiddleToLeftSwitch extends CommandGroup
{
	...
	public MiddleToLeftSwitch()
	{
		...
		addSequential(new DriveOneFoot(Drive.TICKSPERFOOT, 1));
		// addSequential(new SleepCommand(1));
		// addSequential(new DriveEncoderTicks(Drive.TICKSPERFOOT * 12.36));
		addSequential(new TurnEncoderDegrees(-45, 5));
		
		addSequential(new DriveEncoderTicks(Drive.TICKSPERFOOT * 7.25));
		addSequential(new TurnEncoderDegrees(45, 5));
		addSequential(new DriveOneFoot((int) (Drive.TICKSPERFOOT * 2.6), 3));
