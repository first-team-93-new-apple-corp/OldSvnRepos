...
public class Team93PIDController implements LiveWindowSendable
{
	...
	private Tolerance m_tolerance; // the tolerance object used to check if on
									// target
	private double m_setpoint = 0.0;// the target location for the robot to end up at
	private double m_error = 0.0;// The distance traveled in one instance of the PIDController
	private double m_result = 0.0;// The speed that will be sent to the speed controller
	private double m_period = kDefaultPeriod;
	private double m_maximumIContribution = 1.0;// Sets the maximum input for I contribution in PID (includes
												// totalError)
	private double m_minimumIContribution = m_maximumIContribution * -1;// Sets the minimum input for I contribution in
																		// PID (includes totalError)
	 ...
	 *            The default is 50ms.
	 */
	public Team93PIDController(double Kp, double Ki, double Kd, double Kf, PIDSource source, PIDOutput output,
			double period)
	{
	 ...
	 *            The default is 50ms.
	 */
	public Team93PIDController(double Kp, double Ki, double Kd, PIDSource source, PIDOutput output, double period)
	{
	 ...
	 *            The PIDOutput object that is set to the output percentage
	 */
	public Team93PIDController(double Kp, double Ki, double Kd, PIDSource source, PIDOutput output)
	{
	...
	private void calculate()
	{
		...
		if (enabled)
		{
			...
			synchronized (this)
			{
				...
				if (m_I != 0)
				{
					double potentialIContribution = (m_totalError + m_error) // changed
																				// potentialIGain
																				// to
																				// potentialIContribution
							* m_I;
					if (potentialIContribution < m_maximumIContribution)
					{ // changed
						// m_maximumOutput
						// to
						// m_maximumIContribution
						if (potentialIContribution > m_minimumIContribution)
						{ // changed
							// m_minimumoutput
							// to
							// m_minimumIContribution
							m_totalError += m_error;
						}
						else
						{
							m_totalError = m_minimumIContribution / m_I;
						}
					}
					else
					{
						m_totalError = m_maximumIContribution / m_I;
					}
				}
	...
	/**
	 * Sets the minimum and maximum for I Contribution
	 * I Contribution is limited because totalError can become too large
	 *
	...
	public synchronized void setIContributionRange(double minimumIContribution, double maximumIContribution)
	{
		...
		{
			throw new BoundaryException("Lower bound is greater than upper bound for input");
		}
