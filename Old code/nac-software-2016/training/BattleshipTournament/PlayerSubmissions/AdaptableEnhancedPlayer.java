 ...
 *
 */
public class AdaptableEnhancedPlayer extends GamePlayer {
	 ...
	 * @param own_ocean
	 */
	public AdaptableEnhancedPlayer() 
	{
		super();
	...
	private GameBoard.FireResult fire(int x, int y)
	{
		...
		System.out.println(getName() + " fires at (" + String.valueOf(x) + ", " + String.valueOf(y) + ")");
	...
	private void updateState(Coordinate coord, GameBoard.FireResult result)
	{
		...
		switch(result.result_type)
		{
			case SHOT_MISS:
				...
				{
					System.out.println("MISSED THE " + TargetShipList.get(0).ship.name() + "!");
				}
				printLog(", 0\n");
				break;
			case SHOT_HIT:
				...
				break;
			case SHOT_SUNK:
				System.out.println("SUNK THE " + result.ship_type.name() + "!");
				printLog(", 1\n");
				updateProbability(coord.x, coord.y, result);
				...
				if ((checkWithin(x + n, y)) && (checkWithin(x + n + l - 1, y)))
				{
					...
					if (valid)
					{
						for(int k = 0; k < l; k++)
						{
							...
							System.out.println("Coordinate (" + (x + n + k) + ", " + y + ") now has Probability = " + getProbabilityWeighted(x + n + k, y) + ".");
				...
				if ((checkWithin(x, y + n)) && (checkWithin(x, y + n + l - 1)))
				{
					...
					if (valid)
					{
						for(int k = 0; k < l; k++)
						{
							...
							System.out.println("Coordinate (" + x + ", " + (y + n + k) + ") now has Probability = " + getProbabilityWeighted(x, y + n + k) + ".");
