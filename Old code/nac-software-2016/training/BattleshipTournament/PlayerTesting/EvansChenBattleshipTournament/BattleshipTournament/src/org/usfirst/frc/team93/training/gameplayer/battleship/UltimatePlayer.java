...
import org.usfirst.frc.team93.training.gameplayer.battleship.DummyPlayer.Coordinate;
 ...
 *
 */
public class UltimatePlayer extends GamePlayer {
	...
	/**
	 * The different types of locations scores there are in the PointScore object.
	 */
	enum LocationScore
	{
		CENTER, RIGHT, RIGHT_UP, UP, UP_LEFT, LEFT, LEFT_DOWN, DOWN, DOWN_RIGHT;
	}
	...
	//the list of all ships in the game Battleship.
	List<Ship.ShipType_t> allShips = new ArrayList<Ship.ShipType_t>();
	...
	int opponentPlacingStrategyTrackerLength = 15;
	
	double edgeProbability = 0.36;
	
	//the hybrid weights of each strategy
	int hybridWeightRandom = 4;
	int hybridWeightEdge = 4;
	
	//define adaptation variables
	PointStrategy test;
	List<Double> positionScores = new ArrayList<Double>();
	int game = 0;
	//implement bayesian inference on it
	/**
	 *  Set 1: Randomly Placed
	 *  Mean = 15.842520633285776
	 *  Stddev = 7.546962614686333
	 *  
	 *  Set 2: Alternating Edge and Random
	 *  Mean = 23.76670008030113
	 *  Stddev = 8.814994083819508
	 *  
	 *  This means that a point falling more likely within Set 1 should be considered same strategy,
	 *  while a point falling more likely within Set 2 should be considered different strategy.
	 *  UGHHHHH This is such a worse way to do it
	...
	{
		super();
		//populate master ship list
		allShips.add(Ship.ShipType_t.SHIP_CARRIER);
		allShips.add(Ship.ShipType_t.SHIP_BATTLESHIP);
		allShips.add(Ship.ShipType_t.SHIP_CRUISER);
		allShips.add(Ship.ShipType_t.SHIP_SUBMARINE);
		allShips.add(Ship.ShipType_t.SHIP_DESTROYER);
		test = new PointStrategy();
	}
	public UltimatePlayer(int player) 
	{
		super(player);
		//populate master ship list
		allShips.add(Ship.ShipType_t.SHIP_CARRIER);
	...
	private void placeShips(PlacingStrategy p)
	{
		...
		{
			case RANDOM:
				placeShipsHybrid(hybridWeightRandom);
				break;
			case CENTER:
				placeShipsHybrid(hybridWeightRandom);
				break;
			case EDGE:
				placeShipsHybrid(hybridWeightEdge);
				break;
			default:
				placeShipsHybrid(hybridWeightRandom);
				break;
	...
	private void placeShipsRandomly()
	{
		...
		{
			while(placeShip(new Ship(new Ship.GridCoord(x, y), getOrientationFromInt(o), allShips.get(i))) != PlaceResult_t.E_OK)
			{
	...
	private void placeShipsRandomlyEnhanced()
	{
		...
		for (int i = 0; i < allShips.size(); i++)
		{
			...
			while(!placed)
			{
				x = random_generator.nextInt(10);
				y = random_generator.nextInt(10);
				o = random_generator.nextInt(4);
				//try a place
				if (checkShipEnhanced(x, y, allShips.get(i), getOrientationFromInt(o), free))
				{
					//see if it was successful
					placed = placeShip(new Ship(new Ship.GridCoord(x, y), getOrientationFromInt(o), allShips.get(i))) == PlaceResult_t.E_OK;
				}
				//if successful...
				if (placed)
				{
					blockSurroundingShip(x, y, allShips.get(i), getOrientationFromInt(o), free);
				}
			}
		}
	}
		...
		for (int i = 0; i < allShips.size(); i++)
		{
			...
			else
			{
				...
				for (int j = 0; (j < clusterPoints.size()) && (!shipPlaced); j++)
				{
					...
					o = random_generator.nextInt(2);
					if (placeShip(new Ship(new Ship.GridCoord(x, y), getOrientationFromInt(o), allShips.get(i))) == PlaceResult_t.E_OK)
					{
						shipPlaced = true;
					}
				}
			...
			int l = getShipLength(allShips.get(i));
	...
	{	
		...
		for (int i = 0; i < allShips.size(); i++)
		{
			...
			if (checkWithinEdge(y, border, strict))
			{
				...
				{
					if (placeShip(new Ship(new Ship.GridCoord(x, y), Orientation_t.OR_LEFT, allShips.get(i))) == PlaceResult_t.E_OK)
					{
						shipPlaced = true;
					}
					if (!shipPlaced)
					{
						if (placeShip(new Ship(new Ship.GridCoord(x, y), Orientation_t.OR_RIGHT, allShips.get(i))) == PlaceResult_t.E_OK)
						{
							shipPlaced = true;
						}
					}
			...
			else if (checkWithinEdge(x, border, strict))
			{
				...
				if (random_generator.nextInt(2) == 0)
				{
					if (placeShip(new Ship(new Ship.GridCoord(x, y), Orientation_t.OR_UP, allShips.get(i))) == PlaceResult_t.E_OK)
					{
						shipPlaced = true;
					}
					if (!shipPlaced)
					{
						if (placeShip(new Ship(new Ship.GridCoord(x, y), Orientation_t.OR_DOWN, allShips.get(i))) == PlaceResult_t.E_OK)
						{
							shipPlaced = true;
						}
					}
	...
	private void placeShipsEdgesEnhanced(int border, boolean strict)
	{
		...
		List<Ship.ShipType_t> shipsToPlace = new ArrayList<Ship.ShipType_t>(allShips);
	...
	private void placeShipsHybrid(int randomShipNumber)
	{
		...
		List<Ship.ShipType_t> randomShips = new ArrayList<Ship.ShipType_t>(allShips);
	...
	private void resetRemainingShips()
	{
		...
		remainingShips = new ArrayList<Ship.ShipType_t>(allShips);
	}
	/**
	...
	private void resetOpponentTracker()
	{
		...
		if (opponentShots.size() > 15)
		{
			game += 1;
			if (game > 1)
			{
				PointScore p = findPointScore(categorizedCoordinateListToTotal(opponentHits));
				printDifferenceScoreStrategy(test, p);
				test = new PointStrategy(p);
				System.out.println("------------------------------");
			}
			else
			{
	...
	public class Coordinate
	{
		...
		/**
		 * Overrides the equals for this object, allowing for comparisons.
		 */
		@Override
	...
	private FiringStrategy getOpponentFiringStrategy(List<Coordinate> shots)
	{
		...
		boolean center = true;
		for (int i = 0; (i < 13) && (i < shots.size()); i++)
		{
		...
		boolean edge = true;
		for (int i = 0; (i < edgeShotThreshold) && (i < shots.size()); i++)
		{
	...
	private PlacingStrategy getOpponentPlacingStrategy(EnumMap<Ship.ShipType_t, List<Coordinate>> hits)
	{
		...
		{
			if ((1 - edgeProbability) * edgeTiles > edgeProbability * (getHitNumber(hits) - edgeTiles))
			{
				p = PlacingStrategy.EDGE;
			}
			else
	...
	private void printProbability()
	{
		...
		for (int i = 0; i < 10; i++)
		{
			for (int j = 0; j < 10; j++)
			{
				print[i][j] = getProbabilityWeighted(i, j);
			}
		}
		//System.out.println(Arrays.deepToString(print));
	}
	
	/*
	 * More adaptability goes here.
	 */
	
	/**
	 * The class for a PointScore object.
	 * It takes the coordinate list, and then calculuates a PointScore representation.
	 * A PointScore represenation is a 5-number representation of a list of points,
	 * which is based on their distances from the 4 corners of the ocean, as well as its distance from the center.
	 * It is useful in determining the differences between two sets of points.
	 */
	class PointScore
	{
		//the sample size.
		...
		//the list of coordinates to process.
		public List<Coordinate> m_coordinates;
		
		//the list of 5 locationScores.
		public EnumMap<LocationScore, Double> scores = new EnumMap<LocationScore, Double>(LocationScore.class);
		
		//the locations from which to measure the locationScores by, the corners and the center.
		public EnumMap<LocationScore, Point> locations = new EnumMap<LocationScore, Point>(LocationScore.class);
		
		public PointScore(List<Coordinate> coordinates)
		{
			m_coordinates = coordinates;
			sample = m_coordinates.size();
			initLocations();
			calculateScores();
		}
		
		/**
		 * Calculates the scores internally.
		 */
		private void calculateScores()
		{
			//reset all scores to 0
			wipeScores();
			//for each Location to process...
			for (LocationScore e: LocationScore.values())
			{
				//for each coordinate
				for (Coordinate c : m_coordinates)
				{
					//add the distance divided by the sample size to get the average distance of each point to a locationPoint
					scores.replace(e, scores.get(e) +
							(Math.hypot((double)c.x - locations.get(e).x, (double)c.y - locations.get(e).y) / (double)sample));
				}
			}
		}
		...
		/**
		 * Initializes the LocationScore's points, the corners and the center.
		 */
		private void initLocations()
		{
			locations.clear();
			locations.put(LocationScore.CENTER, new Point(4.5, 4.5));
			locations.put(LocationScore.RIGHT, new Point(9, 4.5));
			locations.put(LocationScore.RIGHT_UP, new Point(9, 0));
			locations.put(LocationScore.UP, new Point(4.5, 0));
			locations.put(LocationScore.UP_LEFT, new Point(0, 0));
			locations.put(LocationScore.LEFT, new Point(0, 4.5));
			locations.put(LocationScore.LEFT_DOWN, new Point(0, 9));
			locations.put(LocationScore.DOWN, new Point(4.5, 9));
			locations.put(LocationScore.DOWN_RIGHT, new Point(9, 9));
		}
		
		/**
		 * The class defining a point for a PointScore.
		 */
		public class Point
		{
			double x;
		...
		/**
		 * Returns the score for that LocationScore
		 * @param e The LocationScore to get the score for
		 * @return the value
		 */
		public double getScore(LocationScore e)
		{
			return scores.get(e);
		}
	}
	...
	/**
	 * The class for a PointStrategy object.
	 * it is essentially a list of PointScores, which calculates the
	 * Five Point Scores for all PointScores into one
	 * aggregated master strategy for what the PointScore
	 * group might represent.
	 */
	class PointStrategy
	{
		//the strategy that this group of PointScores represents
		List<PointScore> strategy;
		int points;
		...
		/**
		 * The constructor for PointStrategy.
		 * It initializes the strategy list.
		 */
		public PointStrategy()
		{
			strategy = new ArrayList<PointScore>();
			wipeScores();
		}
		public PointStrategy(PointScore p)
		{
			this();
			add(p);
		}
		...
		/**
		 * Add a PointScore to the strategy.
		 * @param p The PointScore to add.
		 */
		public void add(PointScore p)
		{
			for (LocationScore e : LocationScore.values())
			{
				double strategySample = points;
				double scoreSample = p.sample;
				double totalSample = strategySample + scoreSample;
				double score = ((strategySample / totalSample) * scores.get(e)) + ((scoreSample / totalSample) * p.scores.get(e));
				
				scores.replace(e, score);
			}
			points += p.sample;
		}
	...
	/**
	 * This converts a list of Coordinates to a PointScore object.
	 * @param coords The list of Coordinates.
	 * @return PointScore
	 */
	public PointScore findPointScore(List<Coordinate> coords)
	{
		return new PointScore(coords);
	}
	
	/**
	 * This converts a list of coordinates categorized by enum ShipType into a normal list of Coordinates.
	 * @param categorized The list of Coordinates categorized by ShipType.
	 * @return The list of Coordinates.
	 */
	public List<Coordinate> categorizedCoordinateListToTotal(EnumMap<Ship.ShipType_t, List<Coordinate>> categorized)
	{
		List<Coordinate> coordList = new ArrayList<Coordinate>();
		for (Ship.ShipType_t e: categorized.keySet()) {
			coordList.addAll(categorized.get(e));
		}
		return coordList;
	}
	
	public double getDifferenceScoreStrategyTotal(PointStrategy strategy, PointScore score)
	{
		double sum = 0;
		for (LocationScore e : LocationScore.values())
		{
			double multiplier = 1;
			double power = 1;
			if (e == LocationScore.CENTER)
			{
				multiplier = 6;
				power = 1.5;
			}
			if ((e == LocationScore.RIGHT) || (e == LocationScore.UP) || (e == LocationScore.LEFT) || (e == LocationScore.DOWN))
			{
				multiplier = 2;
				power = 1.2;
			}
			sum += multiplier * Math.pow(Math.abs(score.getScore(e) - strategy.getScore(e)), power);
		}
		return sum;
	}
	
	public void printDifferenceScoreStrategy(PointStrategy strategy, PointScore score)
	{
		System.out.println("POINT: " + score.scores);
		System.out.println("STRATEGY: " + strategy.scores);
		for (LocationScore e : LocationScore.values())
		{
			System.out.println(e.name() + ": " + Math.round((score.getScore(e) - strategy.getScore(e)) * 100.0) / 100.0 + " units of difference.");
		}
		System.out.println("A Difference-Square-Sum of " + Math.round(getDifferenceScoreStrategyTotal(strategy, score) * 100.0) / 100.0 + " was obtained.");
		positionScores.add(getDifferenceScoreStrategyTotal(strategy, score));
		System.out.println(positionScores);
	}
	
	/**
	 * Compare two PointScores.
	 * @param A The first PointScore to compare.
	 * @param B The second PointScore to compare.
	 * @return The probability that they are identical.
	 */
	public double comparePointScore(PointScore A, PointScore B)
	{
		//initialize probability.
		double p = 0;
		
		//First, check points to see if they have a large number of matches.
		int matches = 0;
		for (int i = 0; i < A.m_coordinates.size(); i++)
		{
			//check if occurrence in other list
			Coordinate a = A.m_coordinates.get(i);
			int containsB = B.m_coordinates.indexOf(a);
			//if contains, add 1 to the number of matches and remove both
			if (containsB != -1)
			{
				A.m_coordinates.remove(a);
				B.m_coordinates.remove(containsB);
				matches += 1;
			}
		}
		//repeat with B
		for (int i = 0; i < B.m_coordinates.size(); i++)
		{
			//check if occurrence in other list
			Coordinate b = B.m_coordinates.get(i);
			int containsA = A.m_coordinates.indexOf(b);
			//if contains, add 1 to the number of matches and remove both
			if (containsA != -1)
			{
				A.m_coordinates.remove(b);
				B.m_coordinates.remove(containsA);
				matches += 1;
			}
		}
