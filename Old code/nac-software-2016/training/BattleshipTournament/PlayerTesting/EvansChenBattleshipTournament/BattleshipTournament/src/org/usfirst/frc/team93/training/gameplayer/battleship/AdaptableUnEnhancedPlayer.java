 ...
 *
 */
public class AdaptableUnEnhancedPlayer extends GamePlayer {
	 ...
	 * @param own_ocean
	 */
	public AdaptableUnEnhancedPlayer() 
	{
		super();
	...
	private void placeShipsRandomlyEnhanced()
	{
		...
		for (int i = 0; i < RemainingShips.size(); i++)
		{
			...
			while(!placed)
			{
				...
				if (placed)
				{
					blockSurroundingShip(x, y, RemainingShips.get(i), getOrientationFromInt(o), free);
				}
			}
		}
	}
	...
	private void placeShipsEdgesEnhanced(int border, boolean strict)
	{
		...
		for (int i = 0; i < RemainingShips.size(); i++)
		{
			...
			int x = random_generator.nextInt(10);
			int y = random_generator.nextInt(10);
			Ship.Orientation_t o = getOrientationFromInt(random_generator.nextInt(4));
			
			//pick an open edge point
			while(!((checkWithinEdge(x, border, strict) || checkWithinEdge(y, border, strict)) && (free[x][y])))
			{
				x = random_generator.nextInt(10);
				y = random_generator.nextInt(10);
			}
			
			//if on the horizontal edge...
			if (checkWithinEdge(y, border, strict))
			{
				//check left then right
				if (random_generator.nextInt(2) == 0)
				{
					if (checkShipEnhanced(x, y, RemainingShips.get(i), Ship.Orientation_t.OR_LEFT, free))
					{
						o = Ship.Orientation_t.OR_LEFT;
						valid = true;
					}
					if (!valid)
					{
						if (checkShipEnhanced(x, y, RemainingShips.get(i), Ship.Orientation_t.OR_RIGHT, free))
						{
							o = Ship.Orientation_t.OR_RIGHT;
							valid = true;
						}
					}
				}
			...
			//if on the vertical edge...
			if (checkWithinEdge(y, border, strict))
			{
				//check up then down
				if (random_generator.nextInt(2) == 0)
				{
					if (checkShipEnhanced(x, y, RemainingShips.get(i), Ship.Orientation_t.OR_UP, free))
					{
						o = Ship.Orientation_t.OR_UP;
						valid = true;
					}
					if (!valid)
					{
						if (checkShipEnhanced(x, y, RemainingShips.get(i), Ship.Orientation_t.OR_DOWN, free))
						{
							o = Ship.Orientation_t.OR_DOWN;
							valid = true;
						}
					}
				}
			...
			{
				//try a placement
				if (placeShip(new Ship(new Ship.GridCoord(x, y), o, RemainingShips.get(i))) == PlaceResult_t.E_OK)
				{
					//if successful, yay.
					blockSurroundingShip(x, y, RemainingShips.get(i), o, free);
				}
				else
				{
					//if unsuccessful, place randomly.
					x = random_generator.nextInt(10);
					y = random_generator.nextInt(10);
					o = getOrientationFromInt(random_generator.nextInt(2));
					while(placeShip(new Ship(new Ship.GridCoord(x, y), o, RemainingShips.get(i))) != PlaceResult_t.E_OK)
					{
						x = random_generator.nextInt(10);
						y = random_generator.nextInt(10);
						o = getOrientationFromInt(random_generator.nextInt(2));
