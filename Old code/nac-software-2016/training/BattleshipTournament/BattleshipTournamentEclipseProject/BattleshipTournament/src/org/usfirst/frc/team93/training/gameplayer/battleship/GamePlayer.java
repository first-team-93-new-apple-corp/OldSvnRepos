...
public abstract class GamePlayer {
	...
	protected FireResult fireShot(Ship.GridCoord coord)
	{
		...
		println(getClass().getSimpleName() + " fires at " + String.valueOf(coord.x) + "," + String.valueOf(coord.y));
		
		FireResult r = m_gameboard_view.fireShot(new Ship.GridCoord(coord));  // If a player uses one GridCoord object, this will protect logs
		if (Tournament.log != null)
		{
			Tournament.log.writeAction(coord.x, coord.y, r.result_type, r.ship_type);
		}
	...
	{
		println(getClass().getSimpleName() + " places ship at " + String.valueOf(ship.getLocation().x) + "," + String.valueOf(ship.getLocation().y));
