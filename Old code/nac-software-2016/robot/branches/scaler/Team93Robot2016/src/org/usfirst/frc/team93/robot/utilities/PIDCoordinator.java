...
public class PIDCoordinator
{
	 ...
	 */
	public PIDCoordinator(PIDController controllerOne, PIDController controllerTwo, PIDSource sourceOne,
	...
	private void incrementSetpoint()
	{
		...
		if (m_direction == 1)
		{
			incrementOne = (distanceToGoOne >= m_stepSize) ? m_stepSize : distanceToGoOne;
			incrementTwo = (distanceToGoTwo >= m_stepSize) ? m_stepSize : distanceToGoTwo;
		} else
		{
			incrementOne = (distanceToGoOne <= m_stepSize) ? -1 * m_stepSize : distanceToGoOne;
			incrementTwo = (distanceToGoTwo <= m_stepSize) ? -1 * m_stepSize : distanceToGoTwo;
		}
	...
	public boolean isFinished()
	{
		...
		boolean twoFinished = m_controllerTwo.getError() <= m_maxError && (m_controllerOne.getSetpoint() == m_setpoint);
