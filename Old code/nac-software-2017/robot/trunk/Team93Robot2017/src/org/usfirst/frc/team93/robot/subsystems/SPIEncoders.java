...
import org.usfirst.frc.team93.robot.commands.drive.UpdateSPI;
 ...
 * @author Evans Chen
 * 
 * This class holds the SPI Encoder values. This class periodically updates
 * itself using a Command, UpdateSPI.
 * 
 * To use a SPI as a PIDSource, use SPIEncoders.getSource(port).
 * 
 * This command was established to act as a "middle man" between hardware
 * transactions and the PIDs. This class exists because it is pointless to deal
 * with concurrent programming's locks, synchronization, and debugging, since it
 * is easier and also fully effective just to have this "middle man" class
 * instead.
 */
public class SPIEncoders extends Subsystem
{
	...
	// The HashMap storing the SPIEncoderPIDSources that perform hardware gets
	// from the SPI.
	public static HashMap<Port, WheelAnglePIDSource> s_sources;
	
	// The HashMap storing SPIEncoderSources, which contain SPI Encoder values
	...
	/**
	 * @codereview josh.hawbaker 3.16.17 Yeesh, this constructor is bonkers. Is
	 * there a better way to call this?
	 */
	 ...
	 */
	public SPIEncoders(WheelAnglePIDSource leftFrontSPI, WheelAnglePIDSource leftBackSPI,
			WheelAnglePIDSource rightFrontSPI, WheelAnglePIDSource rightBackSPI, MovingDummyPIDSource leftFrontSource,
			MovingDummyPIDSource leftBackSource, MovingDummyPIDSource rightFrontSource,
			MovingDummyPIDSource rightBackSource)
	{
	...
	/**
	 * Update the SPIEncoderSource values from the SPIEncoderPIDSource values on
	 * a specific port.
	 * 
	 * @param port
	 */
