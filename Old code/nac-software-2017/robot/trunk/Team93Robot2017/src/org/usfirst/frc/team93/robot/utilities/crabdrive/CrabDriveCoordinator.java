...
public class CrabDriveCoordinator
{
	 ...
	 * of robot's angle
	 * 
	 * Alignment: Wheels are at 45 degree angles. No dampening. Easy to line
	 * things up.
	 */
	public enum CrabDriveMode
	{
		RobotCentric, FieldCentric, Alignment, Defense;
	}
	...
	// field centric or robot centric during alignment/defense mode
	public CrabDriveMode m_altMode;
	
	// the turning mode to use
	public CrabTurnMode m_turnMode;
	
	// Yaw PIDController
	public PIDController m_yawController;
	...
	private void update(double speed, double direction, double yaw)
	{
		...
		// if we're in alignment mode, ignore the normal update code.
		if (m_mode == CrabDriveMode.Alignment)
		{
			requestAlign(speed, direction, robotAngle);
			return;
		}
		
		// if we're in defense mode, ignore the normal update code.
		else if (m_mode == CrabDriveMode.Defense)
		{
			requestDefense(speed, direction);
			return;
		}
		...
		double orientation = Drive.getOrientation();
		if (m_mode == CrabDriveMode.FieldCentric)
		{
			orientation = 1.0;
		}
		...
		if (translating)
		{
			...
			SmartDashboard.putBoolean("TURN MODE", false);
		}
		// if robot DOES need to rotate AND it is stationary, use pivot turn.
		else
		{
			requestPivot(speed, robotAngle);
		}
	}
	
	private void requestPivot(double speed, double yawDeviation)
	{
		...
		// limit PID until on target
		wheelsSetTolerance(20.0);
		if (wheelsOnTarget())
		{
		...
		{
			m_yawController.setOutputRange(-0.2, 0.2);
			SmartDashboard.putBoolean("WHEELS ALIGNED", false);
		}
		SmartDashboard.putBoolean("TURN MODE", true);
		// do a crab rotate
	...
	/**
	 * Updates according to alignment mode.
	 */
	private void requestAlign(double speed, double direction, double yawDeviation)
	{
		// if we're pivoting
		if ((Math.abs(yawDeviation) > 2.0) || (Math.abs(speed) < 0.1))
		{
			// set all wheels to pivot mode
			for (Port port : Port.values())
			{
				m_directions.get(port).set(m_crabRotateOffsets.get(port));
			}
		...
		// check whether wheels are in position
		wheelsSetTolerance(20.0);
		if (wheelsOnTarget())
		{
			// if they're correct then we are allowed to move
			m_yawController.setOutputRange(-1.0, 1.0);
			SmartDashboard.putBoolean("WHEELS ALIGNED", true);
		}
		else
		{
			// if they're not yet in position then do nothing.
			SmartDashboard.putBoolean("WHEELS ALIGNED", false);
			m_angleAdjuster.speedReceiver.set(0);
			m_angleAdjuster.yawReceiver.set(0);
			return;
		}
		...
		m_angleAdjuster.speedReceiver.set(0);
		m_angleAdjuster.setDirection(0);
		if (Math.abs(yawDeviation) > 2.0)
		{
			// turn
			m_yawController.setSetpoint(Drive.GYRO.get() + yawDeviation);
			m_yawController.enable();
			// do not allow translation, so end
			return;
		}
		// if translate
		else
		{
			// translate ONLY cardinal directions
			m_yawController.disable();
			// allow translation. Do not end.
		}
		
		// if there's no translation speed, just do nothing
		...
		// allow translation
		// set wheels to translation positions if ordered
		double wheelDirection = direction;
		if (m_altMode == CrabDriveMode.RobotCentric)
		{
			/**
			 * Robot Centric: Wheels are set relative to robot, no offset needed
			 */
		...
		else if (m_altMode == CrabDriveMode.FieldCentric)
		{
			/**
			 * Field Centric: Wheels are set relative to field by subtracting
			 * robot angle
		...
		for (Port port : Port.values())
		{
			m_directions.get(port).set(wheelDirection);
		}
		// turn the drive wheels
		m_angleAdjuster.speedReceiver.set(speed);
	}
	
	/**
	 * Updates according to defense mode.
	 */
	private void requestDefense(double speed, double direction)
	{
		...
		// we never want yaw to change, a change will cause a brown-out
		m_angleAdjuster.yawReceiver.set(0);
		m_yawController.disable();
		// check whether wheels are in position
		wheelsSetTolerance(20.0);
	...
	{
		return m_yawController.getError();
	}
	
	/**
	 * Sets the tolerance for the wheelsOnTarget method.
	 * 
	 * @param tolerance
	 */
	public void wheelsSetTolerance(double tolerance)
	{
	...
	public boolean wheelsOnTarget()
	{
		...
		{
			CrabDriveDirectionMotor wheel = Drive.CRAB_DRIVE_COORDINATOR.m_directions.get(p);
			// if any wheel isn't on target, we're not on target
			if (!wheel.onTarget())
			{
				onTarget = false;
			}
	...
	/**
	 * Gets the closest Cardinal Direction to an angle.
	 * 
	 * @param dir
	 * @return
	 */
	private CardinalDirection closestCardinalDirection(double dir)
	{
	...
	{
		CardinalDirection closest = closestCardinalDirection(dir);
