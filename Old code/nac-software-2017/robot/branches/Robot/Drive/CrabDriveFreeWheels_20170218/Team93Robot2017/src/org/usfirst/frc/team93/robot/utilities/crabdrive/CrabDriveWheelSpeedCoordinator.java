...
public class CrabDriveWheelSpeedCoordinator {
	...
	/**
	 * Constructor for the CrabDriveWheelSpeedCoordinator class.
	 * @param leftFront
	 * @param leftBack
	 * @param rightFront
	 * @param rightBack
	 */
	public CrabDriveWheelSpeedCoordinator(PIDOutputExtended leftFront, PIDOutputExtended leftBack, PIDOutputExtended rightFront, PIDOutputExtended rightBack)
	{
		//initializes the lock
		lock = new Object();
		
		//initializes the receiver PIDOutputs
		yawReceiver = new YawReceiver();
	...
	/**
	 * @author Evans Chen
	 * Put this class as the PIDOutput to a distance PIDController that uses the encoders as a PIDSource.
	 * OR, set this directly from joystick values.
	 */
	public class SpeedReceiver implements PIDOutput
	{
	...
	/**
	 * @author Evans Chen
	 * Put this class as the PIDOutput to a YawController PIDCotroller that uses the gyro as a PIDSource.
	 */
	public class YawReceiver implements PIDOutput
	{
	...
	private void update()
	{
		...
		{
			double speed = speedReceiver.speed;
			double yaw = yawReceiver.yaw;
			PIDOutputExtended motor = m_motors.get(port);
			
			//This block accesses the m_left and m_right lists.
			//This cannot run while we are modifying those lists in setDirection(double dir)
			synchronized(lock)
			{
	...
	/**
	 * Uses the requested direction to determined which wheels are the right and left.
	 * @param dir
	 */
