...
public class SPIEncoderPIDSource implements WheelAnglePIDSource
{
	...
	public double pidGet()
	{
		...
		//then the only check needed to see if it's valid is if it's under 4096.
		else if (!m_reset)
		{
			m_misreads = 0;
		}
		...
		if (m_initialized)
		{
			...
			//otherwise, reading is valid. Clear counter of consecutive misreads.
			else
			{
				m_misreads = 0;
			}
		...
		{
			//if read value gives data other than default 0, AND the resetting is finished, then the encoder is fully initialized.
    ...
    /**
     * Converts a List of Bytes to its string representation.
    ...
    /**
     * Converts a List of Bytes to its hex representation.
     * This will output something in the format "[00, AF, 45, 01]"
     * The list {3, 6, 8} would output:
     * "[03, 06, 08]"
     * @param bytes The List<Byte> byte list.
     * @return The string representation.
     */
    private String byteListToHexString(List<Byte> bytes)
    {
    	//make sure list is not null
    	if(bytes == null)
    	{
    	...
    	for (int i = 0; i < bytes.size(); i++)
    	{
    		...
    		s = s + byteToHexString(bytes.get(i));
    	}
    	s = s + "]";
    	return s;
    }
    ...
    /**
     * Converts a byte into a string hexadecimal representation of the byte.
     * For example, 16 would become "10", and 255 would become "FF".
     * @param b The byte
     * @return The string representation.
     */
