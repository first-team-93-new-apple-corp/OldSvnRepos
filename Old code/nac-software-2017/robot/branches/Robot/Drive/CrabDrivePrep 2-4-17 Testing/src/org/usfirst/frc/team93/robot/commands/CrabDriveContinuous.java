...
public class CrabDriveContinuous extends Command {
    ...
    protected void execute()
    {
    	...
    	if (OI.yaw.get() != 0)
    	{
    		requestAngle = Drive.GYRO.get() + (OI.yaw.get() * multiplier);  // @review NJL Needs to be integrated
    	}
    	else
    	{
    	...
    	{
    		requestAngle = 0.0;         // @review NJL Appears ineffective.  Need to affect gyro.
    	}
    	...
    	double robotAngle = closestAngle(Drive.GYRO.get(), requestAngle);  // @review NJL Hmm...
    	...
    	if (m_mode == CrabDriveMode.RobotCentric)
    	{
	    	 ...
	    	 */
	    	Drive.YawController.setSetpoint(Drive.GYRO.get() + robotAngle);  // robot relative to field
	    	Drive.DirectionController.setSetpoint(Drive.DRIVE_DIRECTION_SPI.get() + wheelAngle);   // wheels relative to robot
    	}
    	...
    	// we think these are the drive wheels
    	Drive.CRAB_DRIVE_LEFT.set(movementOrientation * OI.movement.magnitude().get() * Drive.getOrientation() + Drive.CRAB_DRIVE_TWIST.getLeftTwistTerm());
    	Drive.CRAB_DRIVE_RIGHT.set(movementOrientation * OI.movement.magnitude().get() * Drive.getOrientation() + Drive.CRAB_DRIVE_TWIST.getRightTwistTerm());
    	System.out.println("Drive Left Front = " + Drive.DRIVE_LEFT_FRONT_TALON.get());
    	System.out.println("Drive Left Back = " + Drive.DRIVE_LEFT_BACK_TALON.get());
    	System.out.println("Drive Right Front = " + Drive.DRIVE_RIGHT_FRONT_TALON.get());
    	System.out.println("Drive Right Back = " + Drive.DRIVE_RIGHT_BACK_TALON.get());
