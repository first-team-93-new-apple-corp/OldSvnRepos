 ...
 * camera information to NetworkTables.
 */
public class CameraServer2
{
	...
	private synchronized String[] getSourceStreamValues(int source)
	{
		...
		for (VideoSink i : m_sinks.values())
		{
			...
			if (source == sinkSource && VideoSink.getKindFromInt(CameraServerJNI.getSinkKind(sink)) == VideoSink.Kind.kMjpeg)
			{
	...
	private static final Pattern reMode = Pattern.compile("(?<width>[0-9]+)\\s*x\\s*(?<height>[0-9]+)\\s+(?<format>.*?)\\s+" + "(?<fps>[0-9.]+)\\s*fps");
	 ...
	 *
	 * <p>
	 * This overload calls {@link #startAutomaticCapture(String, int)} with a
	 * name of "USB Camera {dev}".
	 *
	 ...
	 *
	 * <p>
	 * This overload calls {@link #addAxisCamera(String, String)} with name
	 * "Axis Camera".
	 *
	...
	/**
	 * Get OpenCV access to the primary camera feed. This allows you to get
	...
	/**
	 * Get OpenCV access to the specified camera. This allows you to get images
	 * from the camera for image processing on the roboRIO.
	 *
