 ...
 */
public class Team93PIDController implements LiveWindowSendable
{
	...
	private Tolerance m_tolerance; // the tolerance object used to check if on
									// target
	private double m_setpoint = 0.0;// the target location for the robot to end
									// up at
	private double m_error = 0.0;// The distance traveled in one instance of the
									// PIDController
	private double m_result = 0.0;// The speed that will be sent to the speed
									// controller
	private double m_period = kDefaultPeriod;
	private double m_maximumIContribution = 1.0;// Sets the maximum input for I
												// contribution in PID (includes
												// totalError)
	private double m_minimumIContribution = m_maximumIContribution * -1;// Sets
																		// the
																		// minimum
																		// input
																		// for I
	...
	private double m_PContribution = 0;
	private double m_IContribution = 0;
	private double m_PotentialIContribution = 0;
	private double m_DContribution = 0;
	private double m_FContribution = 0;
	...
	private class PIDTask extends TimerTask
	{
		...
		private Team93PIDController m_controller;
		
		public PIDTask(Team93PIDController controller)
		{
			if (controller == null)
			{
	 ...
	 * @param KmaximumIContribution
	 *            the maximum value that I * totalError can contribute to final
	 *            motor value
	 * @param source
	 ...
	 *            The default is 50ms.
	 */
	public Team93PIDController(double Kp, double Ki, double Kd, double Kf, double KmaximumIContribution, PIDSource source, PIDOutput output, double period)
	{
		...
		m_D = Kd;
		m_F = Kf;
		setIContributionRange(KmaximumIContribution * -1.0, KmaximumIContribution);
	 ...
	 *            The default is 50ms.
	 */
	public Team93PIDController(double Kp, double Ki, double Kd, double KmaximumIContribution, PIDSource source, PIDOutput output, double period)
	{
		this(Kp, Ki, Kd, 0.0, KmaximumIContribution, source, output, period);
	}
	 ...
	 *            The PIDOutput object that is set to the output percentage
	 */
	public Team93PIDController(double Kp, double Ki, double Kd, double KmaximumIContribution, PIDSource source, PIDOutput output)
	{
		this(Kp, Ki, Kd, KmaximumIContribution, source, output, kDefaultPeriod);
	}
	 ...
	 *            The PIDOutput object that is set to the output percentage
	 */
	public Team93PIDController(double Kp, double Ki, double Kd, double Kf, double KmaximumIContribution, PIDSource source, PIDOutput output)
	{
		this(Kp, Ki, Kd, Kf, KmaximumIContribution, source, output, kDefaultPeriod);
	}
	...
	private void calculate()
	{
		...
		if (enabled)
		{
			...
			synchronized (this)
			{
				...
				m_PotentialIContribution = (m_totalError + m_error) * m_I;
				
				if (m_I != 0.0)
				{
					// changed potentialIgain to potentialIContribution
					
					if (m_PotentialIContribution < m_maximumIContribution)
					{
						// changed m_maximumoutput to m_maximumIContribution
						
						if (m_PotentialIContribution > m_minimumIContribution)
						{
							// changed m_minimumoutput to m_minimumIContribution
							m_totalError += m_error;
							m_IContribution = m_PotentialIContribution;
						}
						else
						{
							m_IContribution = m_minimumIContribution;
						}
					}
					else
					{
						m_IContribution = m_maximumIContribution;
					}
				}
				
				m_PContribution = m_P * m_error;
				m_DContribution = m_D * (m_error - m_prevError);
				m_FContribution = m_F * m_setpoint;
				
				// Formula
				m_result = m_PContribution + m_IContribution + m_DContribution + m_FContribution;
				m_prevError = m_error;
				// Here's where it makes sure that accumulative isn't greater
				// than max or lower than min
	...
	public double getFCont()
	{
		return (m_FContribution);
	}
	...
	public double getPotentialIContribution()
	{
		return (m_PotentialIContribution);
	}
	
	public double getMaximumIContribution()
	{
		return (m_maximumIContribution);
	}
	
	public double getMinimumIContribution()
	{
		return (m_minimumIContribution);
	}
	...
	public synchronized void setOutputRange(double minimumOutput, double maximumOutput)
	{
		...
		{
			throw new BoundaryException("Lower bound is greater than upper bound for output");
		}
	...
	/**
	 * Sets the minimum and maximum for I Contribution I Contribution is limited
	 * because totalError can become too large
	 *
	 * @param minimumIContribution
	 *            the minimum I Contribution to PID value
	 * @param maximumIContribution
	 *            the maximum I Contribution to PID value
	 */
	public synchronized void setIContributionRange(double minimumIContribution, double maximumIContribution)
	{
		if (minimumIContribution > maximumIContribution)
		{
			throw new BoundaryException("Lower bound is greater than upper bound for I Contribution");
		}
		m_minimumIContribution = minimumIContribution;
		m_maximumIContribution = maximumIContribution;
	}
	 ...
	 * stopping.
	 */
	// @Override there was a problem with disable
