...
public class VisionProcessorUDPReceiver extends Command
{
	...
	// the enum that represents the status of the data.
	// For example, throw out data if tape is not seen.
	public enum DataStatus
	{
		OK(0), TAPE_NOT_SEEN(1);
		
		private final int m_status;
		
		DataStatus(final int status)
		{
			m_status = status;
		}
	...
	private int FORWARD_INDEX = 0;
	private int CENTER_INDEX = 1;
	private int STATUS_INDEX = 2;
	...
	protected void execute()
	{
		...
		if (UDPReceiver != null)
		{
			...
			if (data != null)
			{
				...
				if ((int) data[STATUS_INDEX] == DataStatus.OK.get())
				{
					// update the data
