...
public class ManualWristWithCargo extends Command {
  private double deadZone = 0.03;

  public ManualWristWithCargo() {
  ...
  protected void execute() {
    ...
    if(Math.abs(OI.operator.getRawAxis(3)) > deadZone){
      System.out.println("Manual Wrist with Cargo");
      ...
      if (CargoManipulator.rotateWrist.getVoltage() < 0.8 ) {
        ...
        CargoManipulator.rotator.set(-0.15);
      }
      else{
