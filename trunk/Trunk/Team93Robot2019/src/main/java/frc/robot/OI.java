...
import frc.robot.commands.ToggleEndgamePID;
import frc.robot.commands.angleWrist;
import frc.robot.commands.hatchOuttake;
import frc.robot.commands.intakeOuttake;
import frc.robot.commands.ohShootServoToggle;
import frc.robot.commands.setDriveTo180;
import frc.robot.commands.toggleEndgameElev;
...
import frc.robot.commands.EndgameServoRun;
...
public class OI {
	...
	public static JoystickButton endgameServos;
	public static JoystickButton toggleEndgamePID;
	public static Boolean hasEndedFrontOffset;

	public static JoystickButton driverHatchOuttake;
	public static JoystickButton activateOhShootServoButton;

	public static JoystickButton toggleEndgameElevatorSet;
	...
	public OI()
	{
		...
		levelOne.whenPressed(new ElevatorSetpoint(91, 32));
		levelTwo.whenPressed(new ElevatorSetpoint(225+95, 254));
		levelThree.whenPressed(new ElevatorSetpoint(460+95, 474));
		levelC.whenPressed(new ElevatorSetpoint(225+95, 182, true));
		...
		endgameServos = new JoystickButton(driver, 10);
		endgameServos.whenPressed(new EndgameServoRun());
		toggleEndgamePID = new JoystickButton(driver, 11);
		toggleEndgamePID.toggleWhenPressed(new ToggleEndgamePID());
		
		driverHatchOuttake = new JoystickButton(driver, 5);
		driverHatchOuttake.whenPressed(new hatchOuttake());

		activateOhShootServoButton = new JoystickButton(driver, 6);
		activateOhShootServoButton.whenPressed(new ohShootServoToggle());
		toggleEndgameElevatorSet = new JoystickButton(operator, 12);
		toggleEndgameElevatorSet.whenPressed(new toggleEndgameElev());
	}
